...
?>
<!DOCTYPE html>
<html lang="<?= setting('App.default_language')??'en'=='pt' ? 'pt-br' : setting('App.default_language')??'en' ?>">
<head>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width,initial-scale=1">
    <title><?= lang("App.dashboard_title") ?> - <?= setting('App.title')??'' ?></title>
    <!-- Favicon icon -->
    ...
    <script>
        let pusher = new Pusher('<?= setting('App.pusher_key')??'' ?>', {
            cluster: '<?= setting('App.pusher_cluster')??'' ?>'
        });
